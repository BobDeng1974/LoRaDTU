/**
 *	@作者：杨思源
 *	@日期：2017年12月22日23:11:35
 *	@功能：初始化 dht11 并提供一个统一接口
 *	@版本：1.0
 *	@修改：no
 *	@接口：Device_node device_dht11;								外设对象句柄
 *	@接口：DHT11_Data_TypeDef DHT11;								更新获取到的数据将会保存在这个结构体里面
 *	@接口：void create_dht11(void)								初始化外设和对象句柄
 *	@接口：short DHT11_Read_TempAndHumidity(void *self)			可以无视，直接调用设备对象的update函数指针获取更新
 *	
 *
 *	@注意事项：	数据引脚为 PE6 
 *
...
//	HAL_GPIO_Init(macDHT11_Dout_GPIO_PORT,&GPIO_InitStruct);	   	//初始化PE6	  
	GPIO_InitTypeDef GPIO_InitStruct;
...
static unsigned char DHT11_ReadByte ( void )
{
	...
	{	  
		...
		if(macDHT11_Dout_IN()==GPIO_PIN_SET)/* x us后仍为高电平表示数据“1” */
		{
			/* 等待数据1的高电平结束 */
